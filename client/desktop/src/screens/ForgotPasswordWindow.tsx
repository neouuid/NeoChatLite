...
export const ForgotPasswordWindow: React.FC<ForgotPasswordWindowProps> = ({
  ...
  const handleSendCode = async () => {
    ...
    try {
      ...
      setResetToken(token);
      Alert.alert('成功', '验证码已发�?);
      setStep('verify');
      startCountdown();
    } catch (error) {
      Alert.alert('错误', error instanceof Error ? error.message : '发送失�?);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleResetPassword = async () => {
    ...
    if (newPassword.length < 6) {
      Alert.alert('提示', '密码至少需�?个字�?);
      return;
    }
    if (newPassword !== confirmPassword) {
      Alert.alert('提示', '两次密码输入不一�?);
      return;
    }
          ...
          <View style={styles.card}>
            ...
            <View style={styles.header}>
              ...
              <View style={styles.headerContent}>
                ...
                {step === 'verify' && (
                  <>
                    <Text style={styles.title}>输入验证�?/Text>
                    <Text style={styles.subtitle}>验证码已发送至 {identifier}</Text>
                  </>
                )}
                {step === 'reset' && (
                  <>
                    <Text style={styles.title}>设置新密�?/Text>
